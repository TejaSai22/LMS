...
app.get('/api/analytics/instructor-load', authorize('Admin'), async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM v_InstructorLoad');
    res.json(rows);
  } catch (err) {
    console.error(err.stack);
    res.status(500).send('Error fetching instructor load');
  }
});

app.get('/api/analytics/course-performance', authorize(['Admin', 'Instructor']), async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM v_CoursePerformance');
    res.json(rows);
  } catch (err) {
    console.error(err.stack);
    res.status(500).send('Error fetching course performance');
  }
});

app.get('/api/students/:studentId/transcript', authorize(['Admin', 'Student']), async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM v_StudentTranscript WHERE StudentID = $1', [studentId]);
    res.json(rows);
  } catch (err) {
    console.error(err.stack);
...
app.post('/api/enrollments/:enrollmentId/grade', authorize('Instructor'), async (req, res) => {
  try {
    const { enrollmentId } = req.params;
    const { NumericGrade, LetterGrade, Comments } = req.body;

    // Verify instructor owns the course offering associated with this enrollment
    const enrollmentResult = await pool.query('SELECT OfferingID FROM Enrollment WHERE EnrollmentID = $1', [enrollmentId]);
    if (enrollmentResult.rows.length === 0) return res.status(404).send('Enrollment not found');
    const { offeringid } = enrollmentResult.rows[0];

    const offeringResult = await pool.query('SELECT InstructorID FROM CourseOffering WHERE OfferingID = $1', [offeringid]);
    if (offeringResult.rows[0].instructorid !== req.user.userId) {
      return res.status(403).send('You are not authorized to grade this enrollment');
    }

    const { rows } = await pool.query(
      'INSERT INTO Grade (EnrollmentID, NumericGrade, LetterGrade, GradeDate, Comments) VALUES ($1, $2, $3, NOW(), $4) RETURNING *',
      [enrollmentId, NumericGrade, LetterGrade, Comments]
    );
    res.status(201).json(rows[0]);
  } catch (err) {
    console.error(err.stack);
...
app.get('/api/enrollments/:enrollmentId/grade', authorize(), async (req, res) => {
  try {
    const { enrollmentId } = req.params;

    // Verify user is the student or the instructor/admin
    const enrollmentResult = await pool.query('SELECT StudentID, OfferingID FROM Enrollment WHERE EnrollmentID = $1', [enrollmentId]);
    if (enrollmentResult.rows.length === 0) return res.status(404).send('Enrollment not found');
    const { studentid, offeringid } = enrollmentResult.rows[0];
    ...
    const { rows } = await pool.query('SELECT * FROM Grade WHERE EnrollmentID = $1', [enrollmentId]);
    if (rows.length === 0) return res.status(404).send('Grade not found');
    res.json(rows[0]);
  } catch (err) {
    console.error(err.stack);
